...
check_required_mcp_servers() {
    ...
    if [ ! -f "$mcp_config" ]; then
        ...
        warn "The Playwright MCP server is required for testing commands:"
        warn "  • /aitt:manual-test - Manual testing with browser automation"
        warn "  • /aitt:e2e-test:plan - E2E test planning (website exploration)"
        warn "  • /aitt:e2e-test:write - Playwright test creation and execution"
        warn ""
    ...
    if ! grep -q '"playwright"' "$mcp_config" 2>/dev/null; then
        ...
        warn "╰─────────────────────────────────────╯"
        warn ""
        warn "Your .mcp.json doesn't include the Playwright MCP server."
        warn "This is optional but required for testing commands:"
        warn "  • /aitt:manual-test - Manual testing with browser automation"
        warn "  • /aitt:e2e-test:plan - E2E test planning (website exploration)"
        warn "  • /aitt:e2e-test:write - Playwright test creation and execution"
        warn ""
