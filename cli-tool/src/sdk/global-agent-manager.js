...
async function createGlobalAgent(agentName, options = {}) {
  ...
  try {
    ...
    if (agentName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/ByborgAI/prompt-collection/main/cli-tool/components/agents/${agentName}.md`;
    } else {
...
async function findAgentUrl(agentName) {
  try {
    ...
    const rootUrl = `https://raw.githubusercontent.com/ByborgAI/prompt-collection/main/cli-tool/components/agents/${agentName}.md`;
    ...
    const categoriesResponse = await fetch('https://api.github.com/repos/ByborgAI/prompt-collection/contents/cli-tool/components/agents');
    ...
    for (const item of contents) {
      if (item.type === 'dir') {
        const categoryUrl = `https://raw.githubusercontent.com/ByborgAI/prompt-collection/main/cli-tool/components/agents/${item.name}/${agentName}.md`;
        try {
